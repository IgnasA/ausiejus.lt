...
directory as the tracker. (Same convention as the Trailhead repo; to wire up a
real tracker, run `/setup-matt-pocock-skills`.)

## Wayfinding operations

- **Maps** are the root-level `tracker/map*.md` files, each labelled
  `wayfinder:map`. One per effort:
  - [A portfolio that survives the skim](map.md) — complete: https://ausiejus.lt is live.
  - [An atlas drawn from memory](map-pixel-atlas.md) — active: a standalone
    travel-map exhibit, rough hand-drawn-looking borders generated from real
    geodata.
